...
// =============================================================
// Panorama — vista agrupada por semáforo de los prospectos activos.
// Vive como tab dentro de Cuentas y también como ruta propia
// (/panorama la envuelve para no romper favoritos).
//
// Paleta oscura fija heredada del mockup original: NO respeta el
// tema claro/oscuro de la app. Es deuda conocida — pasarlo a
// Tailwind con la clase `dark` implicaría reescribir todo el CSS.
//
// Sin IA: solo lectura vía /api/panorama. El fetch corre en un
// useEffect al montar, así que el endpoint (el más pesado de la
// app) solo se dispara cuando el componente se monta de verdad.
// =============================================================
...
export function PanoramaSeccion({ onVerPipeline }: { onVerPipeline: () => void }) {
  ...
  return (
    <div className="pan">
      {/* CSS del mockup de referencia, scoped bajo .pan.
          dangerouslySetInnerHTML evita el hydration mismatch de React con
          nodos de texto dentro de <style>.

          Ajustado de página a PANEL: se quitaron min-height:100vh,
          max-width:900px y margin:0 auto, y se agregaron borde y esquinas
          redondeadas. Así se integra dentro del tab de Cuentas en vez de
          leerse como una pantalla completa metida a la fuerza. */}
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .pan .sub{color:var(--muted);font-size:13px;margin-bottom:18px}
        ...
        @media(max-width:560px){
          .pan{padding:12px;border-radius:12px}
          .pan .resumen{gap:6px}
      ...
      {/* Sin <h1>: la pantalla Cuentas ya tiene el suyo y dos títulos
          competían. La línea de conteo sí aporta y se conserva. */}
      <div className="sub">
        {cargando
          ? "Cargando prospectos..."
      ...
      {cargando ? (
        ...
        <div className="estado-msg">
          ...
          <p>Cuando tengas empresas en el pipeline aparecerán acá con su semáforo de atención.</p>
          <button className="btn-retry" onClick={onVerPipeline}>Ver pipeline</button>
        </div>
      ) : (
        <>
